...
/**
 * Shared demo/seed routine. Used by both the CLI (`prisma/seed.ts`) and the
 * guarded `/api/setup?demo=1` endpoint so a fresh production database can be
 * populated without a local shell. All writes are idempotent upserts.
 */
...
export async function runSeed(prisma: PrismaClient): Promise<string[]> {
  ...
  // ---- Admin + demo customer -------------------------------------
  const adminEmail = (process.env.SEED_ADMIN_EMAIL || 'admin@tinytreasurecompetitions.com').toLowerCase();
  ...
  log.push(`Admin ready: ${admin.email}`);
  ...
  log.push(`Demo customer: ${customer.email}`);
  ...
  log.push(`${competitions.length} competitions`);
  ...
  log.push('1 drawn competition + winner');
  ...
  for (const p of posts) {
    ...
    await prisma.blogPost.upsert({
      where: { slug: p.slug },
      update: {},
      create: { ...rest, published: true, publishedAt: new Date(), categoryId: categoryMap[category] },
    });
  }
  log.push(`${posts.length} blog posts`);

  // ---- FAQs -------------------------------------------------------
  const faqs = [
    { question: 'How do I enter a competition?', answer: 'Choose a competition, answer the skill question, select how many entries you’d like, and complete your purchase securely with Stripe. Your ticket numbers are allocated instantly once payment is confirmed.', category: 'Entering', order: 1 },
    { question: 'When and how is the winner drawn?', answer: 'Each competition has a published draw date. Winners are selected using a verifiable random method from all valid entries, and every winner is published on our Winners page.', category: 'Draws', order: 2 },
    { question: 'Is there a free postal entry route?', answer: 'Yes. In line with UK law, a free entry route is available by post. See our Terms & Conditions for the current postal entry address and requirements.', category: 'Entering', order: 3 },
    { question: 'How will I receive my prize?', answer: 'Physical prizes are delivered free of charge to your UK address. Cash prizes are paid directly to your bank account, usually within 7 days of the draw.', category: 'Prizes', order: 4 },
    { question: 'Can I get a refund?', answer: 'Entries are generally non-refundable once a competition has closed. If a competition is cancelled, all entrants are refunded in full. Contact us if you believe there has been an error.', category: 'Payments', order: 5 },
  ];
  for (const f of faqs) {
  ...
  log.push(`${Object.keys(settings).length} site settings`);

  // ---- Reviews ----------------------------------------------------
  const reviews = [
    { name: 'Hannah T.', location: 'Leeds', quote: 'Won a full nursery set for my second baby. The whole process was so easy and delivery was quick!', rating: 5, order: 1 },
    { name: 'Priya K.', location: 'Birmingham', quote: 'Love that they publish every winner. Feels genuinely trustworthy compared to other sites.', rating: 5, order: 2 },
    { name: 'James & Leah', location: 'Bristol', quote: 'Cheaper than buying a travel system outright and we actually won one. Over the moon!', rating: 5, order: 3 },
  ];
  ...
  log.push(`${reviews.length} reviews`);
  ...
  log.push(`${blocks.length} content blocks`);
  ...
  for (const p of pages) {
    await prisma.page.upsert({ where: { slug: p.slug }, update: p, create: { ...p, published: true } });
  }
  log.push(`${pages.length} content pages`);
